...
PoolAllocator::PoolAllocator(MemoryManager& mng, size_t blockSize, size_t amount)
...
PoolAllocator::PoolAllocator(MemoryManager& mng, Allocator& alloc, size_t blockSize, size_t amount)
	: m_currentlyAllocated(0)
	, m_blockSize(memory_tools::align_bytes(blockSize))
	, m_poolSize(m_blockSize* amount)
	, m_allocator(alloc)
